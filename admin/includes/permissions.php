...
class PermissionManager {
    ...
    // New method to check if user can perform action on current page
    ...
    // New method to show/hide buttons based on permissions
    public function showActionButton($action, $buttonText, $url = '#', $confirmMessage = '') {
        if ($this->canPerformAction($action)) {
            ...
            return "<button class='btn btn-secondary' disabled title='No permission to $action'>$buttonText</button>";
        }
    }
    
    // New method to show/hide form elements
    public function showFormField($action, $fieldHtml) {
        if ($this->canPerformAction($action)) {
            return $fieldHtml;
        } else {
            return "<div style='opacity: 0.6; cursor: not-allowed;' title='No permission to edit'>$fieldHtml</div>";
        }
    }
    ...
    public function getModuleFromPage($pageName) {
        $pageToModuleMap = [
            ...
            'foster-details.php' => 'foster_info',
            'new-foster.php' => 'foster_info',
            'matchmaking.php' => 'foster_info',
            'schedule.php' => 'schedule',
            'reports.php' => 'reports',
            'settings.php' => 'settings',
            'request-role.php' => 'dashboard', // Fixed this line
            'edit-user.php' => 'user_management',        // Added this line
            'add-user.php' => 'user_management',
            // Add superadmin pages
            ...
            'role-requests.php' => 'role_permissions',
